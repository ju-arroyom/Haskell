...
{-
  File      :  Main.hs
  Copyright : (c) Juan Arroyo, 11/16/17
  Contains functions for producing electoral
  map of the USA
-}
...
-- Translate point in a Subregion to the coordinates of the bounding area
...
main = do
    ...
    args <- getArgs
    all_files <- getDirectoryContents "purple/"
    let region = (args !! 0) ++ ".txt"
    if region `elem` all_files
        then do
            ...
            when response $ do
              if val == 1 || val == 2
                then do
                  ...
                  -- let voteInput = helperVoting voteStrings
